...
begin
  ...
  begin
    try
      dm1.q1.SQL.Text := 'select * from tb_locais_estoque where loc_codigo = :codigo';
      dm1.q1.ParamByName('codigo').Value := StrToInt(edc_cod.Text);

      dm1.q1.Open;

      CodEdit.Text := dm1.q1.FieldByName('loc_codigo').Value;
      DescricaoEdit.Text := dm1.q1.FieldByName('loc_descricao').Value;

      StatusBox.ItemIndex := StatusBox.Items.IndexOf(dm1.q1.FieldByName('loc_status').Value);
...
begin
  ...
  begin
    dm1.q1.SQL.Clear;
    dm1.q1.SQL.Add('update tb_locais_estoque set ');
    dm1.q1.SQL.Add('loc_descricao = :loc_descricao, loc_status = :loc_status ');
    dm1.q1.SQL.Add('where loc_codigo = :loc_codigo');
    dm1.q1.ParamByName('loc_codigo').Value := CodEdit.Text;
    msg_confirma := 'Salvar alterações?';
  end;

  dm1.q1.ParamByName('loc_descricao').Value := DescricaoEdit.Text;
  dm1.q1.ParamByName('loc_status').Value := StatusBox.Text;
