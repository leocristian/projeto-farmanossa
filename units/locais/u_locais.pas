...
uses u_infoLocal, u_dm1, u_controleForm;

procedure TPagLocais.Detalhar1Click(Sender: TObject);
begin
  if not tb_locais.Active then exit;
  if tb_locais.RecordCount = 0 then exit;
...
begin
  ...
  begin
    try
      ...
      index := gridLocaisDBTableView1.DataController.GetSelectedRowIndex(0);
      codigo := gridLocaisDBTableView1.ViewData.Records[index].Values[0];

      q1.SQL.Text := 'delete from tb_locais_estoque where loc_codigo = :codigo';
