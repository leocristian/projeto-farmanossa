...
uses
  ...
  Vcl.StdCtrls, Vcl.Buttons, u_dm1;
...
begin
  ...
  qOpe.Connection := dm1.con1;

  qOpe.Close;
  qOpe.Connection := dm1.con1;
  qOpe.SQL.Text := 'select ope_codigo, ope_nome from tb_operadores';
  qOpe.Open;
end;
...
begin
  ...
  begin
    xWhere := xWhere + 'and ope_codigo = ' + strBusca;
  end
  else if FrameBusca1.BuscaSelect.Text = 'NOME COMPLETO' then
  begin
    xWhere := xWhere + 'and ope_nome like ' + QuotedStr('%' + strBusca + '%');
  end;

  qOpe.SQL.Clear;
  qOpe.SQL.Add('select ope_codigo, ope_nome from tb_operadores where 1=1 ' + xWhere);
  qOpe.Open;
...
procedure TPagOperador.FrameBusca1SpeedButton1Click(Sender: TObject);
begin
  Close;
end;
