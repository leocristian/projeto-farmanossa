...
begin
  ...
  if (FrameButtons.ModoEdit.Text = 'V') or (FrameButtons.ModoEdit.Text = 'A') then
  begin
    try
      dm1.q1.SQL.Text := 'select ope_codigo, ope_nome from tb_operadores where ope_codigo = :codigo';
      dm1.q1.ParamByName('codigo').Value := StrToInt(edc_cod.Text);

      dm1.q1.Open;

      CodEdit.Text := dm1.q1.FieldByName('ope_codigo').Value;
      NomeEdit.Text := dm1.q1.FieldByName('ope_nome').Value;
    finally
      dm1.q1.Close;
    end;
...
begin
  try
    ...
    begin
      ...
      dm1.q1.SQL.Text := 'select nextval(''tb_operadores_cod_seq'') as ope_cod';

      dm1.q1.Open;
      ope_cod := dm1.q1.FieldByName('ope_cod').Value;
      dm1.q1.Close;

      dm1.q1.SQL.Clear;

      dm1.q1.SQL.Add('insert into tb_operadores');
      dm1.q1.SQL.Add(' values (:ope_codigo, :ope_nome, md5(:ope_login), md5(:ope_senha))');

      dm1.q1.ParamByName('ope_codigo').Value := ope_cod;
      dm1.q1.ParamByName('ope_nome').Value := NomeEdit.Text;
      dm1.q1.ParamByName('ope_login').Value := AdicionarSemente(LoginEdit.Text);
      dm1.q1.ParamByName('ope_senha').Value := AdicionarSemente(SenhaEdit.Text);
    end
    else if FrameButtons.ModoEdit.Text = 'A' then
    begin
      ...
      dm1.q1.SQL.Clear;
      dm1.q1.SQL.Add('update tb_operadores set ');
      dm1.q1.SQL.Add('ope_nome = :ope_nome');
      dm1.q1.SQL.Add('where ope_codigo = :ope_codigo');
      dm1.q1.ParamByName('ope_nome').Value := NomeEdit.Text;
      dm1.q1.ParamByName('ope_codigo').Value := CodEdit.Text;
    end;
