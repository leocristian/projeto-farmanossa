...
uses u_controleForm, u_dm1, u_selecionaLote, u_saidas,
  u_buscaProduto, u_buscaLocal;
...
begin
  ...
  begin
    Aviso('Informe o Produto e o Local de Estoque!');
    CodProdEdit.SetFocus;
    Exit;
  end;
...
begin
  ...
  try
    dm1.q1.SQL.Text := 'select prod_descricao, prod_status, prod_status_saida from tb_produtos where prod_codigo = :codigo';
    ...
    if dm1.q1.FieldByName('prod_status').Value = 'INATIVO' then
      ...
      else if dm1.q1.FieldByName('prod_status_saida').Value = 'INATIVO' then
      begin
        Aviso('O produto ' + dm1.q1.FieldByName('prod_descricao').Value + ' não permite saida!');
        CodProdEdit.Clear;
        DescProdEdit.Clear;
        CodProdEdit.SetFocus;
      end
...
procedure TFormSaida.LimparLocalClick(Sender: TObject);
begin
  CodLocalEdit.Clear;
  DescLocalEdit.Clear;
end;

procedure TFormSaida.LimparProdClick(Sender: TObject);
begin
  CodProdEdit.Clear;
  DescProdEdit.Clear;
end;
