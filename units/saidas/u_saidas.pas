...
type
  TPagSaidas = class(TForm)
    ...
    pn_busca: TPanel;
    FrameGrid2: TFrameGrid;
    dataBox: TGroupBox;
    Label1: TLabel;
    Label2: TLabel;
    LimparDatasBtn: TButton;
    DataInicialEdit: TcxDateEdit;
    DataFinalEdit: TcxDateEdit;
    CodigoBox: TGroupBox;
    Label3: TLabel;
    Label4: TLabel;
    CodInicialEdit: TNumberBox;
    CodFinalEdit: TNumberBox;
    Button2: TButton;
    BuscarBtn: TBitBtn;
    MostrarTudoBtn: TButton;
    ...
    procedure DataFinalEditClick(Sender: TObject);
    procedure LimparDatasBtnClick(Sender: TObject);
    procedure CodInicialEditClick(Sender: TObject);
    procedure CodFinalEditClick(Sender: TObject);
...
begin
  ...
  if (DataInicialEdit.Text = '') or (DataFinalEdit.Text = '') then
  begin
    Aviso('Informe um período válido!');
    Exit;
  end;

  if (CodInicialEdit.Value = 0) or (CodFinalEdit.Value = 0) then
  begin
    Aviso('Informe o código para busca!');
    Exit;
  end;
  ...
  tb_saidas.SQL.Add('where (sai_data_hora::date >= :dataInicial');
  tb_saidas.SQL.Add('and sai_data_hora::date <= :dataFinal)');
  tb_saidas.SQL.Add('and (sai_codigo >= :codigoInicial and sai_codigo <= :codigoFinal)');

  tb_saidas.ParamByName('dataInicial').Value := DataInicialEdit.Date;
  tb_saidas.ParamByName('dataFinal').Value := DataFinalEdit.Date;
  tb_saidas.ParamByName('codigoInicial').Value := CodInicialEdit.Value;
  tb_saidas.ParamByName('codigoFinal').Value := CodFinalEdit.Value;
...
procedure TPagSaidas.Button2Click(Sender: TObject);
begin
  CodInicialEdit.Clear;
  CodFinalEdit.Clear;
end;
...
procedure TPagSaidas.CodFinalEditClick(Sender: TObject);
begin
  CodFinalEdit.SetFocus;
end;

procedure TPagSaidas.CodInicialEditClick(Sender: TObject);
begin
  CodInicialEdit.SetFocus;
end;

procedure TPagSaidas.DataFinalEditClick(Sender: TObject);
begin
  DataFinalEdit.SetFocus;
end;

procedure TPagSaidas.DataInicialEditClick(Sender: TObject);
begin
  DataInicialEdit.SetFocus;
end;
...
procedure TPagSaidas.LimparDatasBtnClick(Sender: TObject);
begin
  DataInicialEdit.Clear;
  DataFinalEdit.Clear;
end;
