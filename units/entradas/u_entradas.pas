...
procedure TPagEntradas.LimparDatasBtnClick(Sender: TObject);
begin
  DataInicialEdit.Clear;
  DataFinalEdit.Clear;
end;

procedure TPagEntradas.MostrarTudoBtnClick(Sender: TObject);
begin
  tb_entradas.Close;
...
begin
  ...
  tb_entradas.SQL.Add('inner join tb_produtos on prod_codigo = ent_produto');
  tb_entradas.SQL.Add('inner join tb_locais_estoque on loc_codigo = ent_local');
  tb_entradas.SQL.Add('where (ent_data_hora::date >= :dataInicial');
  tb_entradas.SQL.Add('and ent_data_hora::date <= :dataFinal)');
  tb_entradas.SQL.Add('and (ent_codigo >= :codigoInicial and ent_codigo <= :codigoFinal)');

  tb_entradas.ParamByName('dataInicial').Value := DataInicialEdit.Date;
  tb_entradas.ParamByName('dataFinal').Value := DataFinalEdit.Date;
  tb_entradas.ParamByName('codigoInicial').Value := CodInicialEdit.Value;
...
procedure TPagEntradas.Button2Click(Sender: TObject);
begin
...
procedure TPagEntradas.CodFinalEditClick(Sender: TObject);
begin
  CodFinalEdit.SetFocus;
end;

procedure TPagEntradas.CodInicialEditClick(Sender: TObject);
begin
  CodInicialEdit.SetFocus;
end;

procedure TPagEntradas.DataFinalEditClick(Sender: TObject);
begin
  DataFinalEdit.SetFocus;
end;

procedure TPagEntradas.DataInicialEditClick(Sender: TObject);
begin
