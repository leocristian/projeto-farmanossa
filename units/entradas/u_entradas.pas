...
begin
  ...
  begin
    dataInicial := DateToStr(FiltroDataCod1.DataInicialEdit.Date);
    dataFinal := DateToStr(FiltroDataCod1.DataFinalEdit.Date);

    xWhere := xWhere + ' and to_char(ent_data_hora::date, ''dd/mm/yyyy'') >= ' + QuotedStr(dataInicial) +
                       ' and to_char(ent_data_hora::date, ''dd/mm/yyyy'') <= ' + QuotedStr(dataFinal);
  end;

  if (FiltroDataCod1.CodInicialEdit.Text <> '') and (FiltroDataCod1.CodFinalEdit.Text <> '') then
  begin
    codInicial := FiltroDataCod1.CodInicialEdit.Text;
    codFinal := FiltroDataCod1.CodFinalEdit.Text;

    xWhere := xWhere + ' and ent_codigo >= ' + codInicial + ' and ent_codigo <= ' + codFinal;
  end;
  ...
  qEnt.SQL.Add('left join tb_produtos on prod_codigo = ent_produto');
  qEnt.SQL.Add('left join tb_locais_estoque on loc_codigo = ent_local');
  qEnt.SQL.Add('where 1=1 ' + xWhere);
