...
begin
  ...
  begin
    xWhere := xWhere + ' and qtd_estoque != 0 ';
  end
  else if QuantidadeBox.ItemIndex = 2 then
  begin
    xWhere := xWhere + ' and qtd_estoque > ' + QuantidadeEdit.Text;
  end
  ...
  begin
    xWhere := xWhere + ' and prod_codigo in (' + CodProdEdit.Text + ') ';
  end;
  if CodLocalEdit.Text <> '' then
  begin
    xWhere := xWhere + ' and loc_codigo in (' + CodLocalEdit.Text + ') ';
  end;

  if TipoProdutoBox.ItemIndex = 0 then
  begin
    xWhere := xWhere + ' and prod_status in (''ATIVO'', ''INATIVO'') ';
  end
  else if TipoProdutoBox.ItemIndex = 1 then
  begin
    xWhere := xWhere + ' and prod_status = ''ATIVO'' ';
  end
  else if TipoProdutoBox.ItemIndex = 2 then
  begin
    xWhere := xWhere +'and prod_status  = ''INATIVO'' ';
  end;
...
begin
  ...
  LotesProdutoForm.prod_cod.Text := qEst.FieldByName('prod_codigo').AsString;
